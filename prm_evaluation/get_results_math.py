...
math_500 = load_dataset("RLHFlow/Mistral-MATH500-Test")['train']

print(math_500)
gt_labels = math_500['label']
print(len(gt_labels[0]))

completion_path = "/path/to/TDRM/evaluation/outputs/math-500/ds_td_lambda_prm_last_ckpt_gamma=1_lam=0.7_all_100_full/output.json"
...
with open(completion_path, "r") as f:
    for idx, line in enumerate(f):
        ...
        for sub_idx, sub_item in enumerate(item["assistant"]):
            item['assistant'][sub_idx]["idx"] = sub_idx
        completions.append(item)

all_prompts = math_500['prompt']
for i in range(len(completions)):
    assert completions[i]['user'] == all_prompts[i]
...
for i in tqdm(range(len(completions))):
    for j in range(len(completions[i]["assistant"])):
        completions[i]["assistant"][j]["answer"] = normalize_final_answer(completions[i]["assistant"][j]["content"].split("The answer is: ")[-1].strip())

best_of_k_acc = []

def cal_acc(label_idx):
    labels = []
    for idx, label_list in zip(label_idx, gt_labels):
        labels.append(label_list[idx])
    return sum(labels) / len(labels)

for i in tqdm(range(len(completions[0]["assistant"]))):
    partial_bon = [completion_item["assistant"][:i+1] for completion_item in completions]
    for j in range(len(partial_bon)):
        partial_bon[j] = sorted(partial_bon[j], key=lambda x: x["reward"])
        # partial_bon[i] = completions[i]['assistant']
    best_of_k_acc.append(cal_acc([x["idx"] for x in [partial_bon[j][i] for j in range(len(partial_bon))]]))
...
def get_pass_at_k(data_path, num=-1):
    ...
    for i in range(len(completions[0]["assistant"])):
        for j in range(len(completions)):
            completion_list[j].append(completions[j]["assistant"][i]["idx"])

        pass_at_k.append(cal_pass(completion_list))
    # pass_list = get_pass_list(completion_list)

    best_of_k_acc = []

    def argmax_answers(preds):
        return max(preds, key=lambda x: x["reward"])["idx"]
    ...
    for i in tqdm(range(len(completions[0]["assistant"]))):
        best_of_k_acc.append(cal_acc([argmax_answers(x["assistant"][:i+1]) for x in completions]))


    return pass_at_k, best_of_k_acc, []

import matplotlib.pyplot as plt

# Sample data for two lists of accuracy

pass_at_k, best_of_k_acc, pass_list = get_pass_at_k(completion_path)

epochs = range(1, len(best_of_k_acc) + 1)

print(best_of_k_acc[-1])
print(f"Pass@1: {best_of_k_acc[0]}")
...
plt.title("Bo1024 on MATH of ORM Qwen", fontsize=16, fontweight='bold')
...
# plt.savefig("/path/to/TDRM/evaluation/bon_math500/llama3.1_td.png")
# Display the chart
plt.show()
