...
def get_score(model, tokenizer, prompts, responses):
    ...
    target_token_ids = tokenizer([" Yes", " No"], add_special_tokens=False).input_ids
    plus_id, minus_id = target_token_ids[0][0], target_token_ids[1][0]
    ...
    for prompt, response in zip(prompts, responses):
        # messages.append([{"role": "user", "content": prompt[0] + response[0]}])
        single_step_score = []
        conversation = []
        forward_conv = []
        # if args.model_type == "Mistral":
        # ans_list = response[0].split("ки\n")
        # else:
        #     ans_list = ans.split("\n\n")
        ...
        conversation.append({"content": "Is the answer correct (Yes/No)? Yes", "role": "assistant"})

        input_ids = tokenizer.apply_chat_template(conversation,return_tensors="pt").to(model.module.device)
        with torch.no_grad():
            logits = model(input_ids).logits[:,-3,:] #simple version, the +/- is predicted by the '-3' position
            scores = logits.softmax(dim=-1)[:,plus_id] # 0 means the prob of + (1 mean -)
            #print(scores)
    ...
    print(f"Score list is {torch.tensor(scores_list)}")
    return torch.stack([torch.tensor(score) for score in scores_list])

    # print(messages)
    # Apply chat template to each message pair and tokenize the input
    # batch_input = tokenizer.apply_chat_template(messages, return_tensors="pt", tokenize=False, add_generation_prompt=True)
...
def main():
    ...
    parser.add_argument('--prm_path', type=str, default='/path/to/TDRM/prm_soft/t1/t11/checkpoint-8538')
    parser.add_argument('--clean', type=str, default='/path/to/TDRM/prm_soft/t1/t11/checkpoint-8538')
