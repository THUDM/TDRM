...
class GRPOCuriosityTrainer(Trainer):
    """
    ...
    def __init__(
        ...
        self.constant_normalizer = constant_normalizer
    ...
    def _generate_and_score_completions(
            ...
            with profiling_context(self, reward_func_name):
                if isinstance(
                    ...
                    if is_conversational(inputs[0]):
                        ...
                        messages = [{"messages": remove_sys(p) + c} for p, c in zip(prompts, completions)]
        ...
        # calculate acc
        # if mode == "eval":
        #     args_for_acc = gather_object(reward_kwargs)
        #     if self.accelerator.is_main_process:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        print(f"Shape of Completion IDs: {completion_ids.shape}")
        decoded_texts = [self.processing_class.decode(ids, skip_special_tokens=True) for ids in completion_ids]

        # Compute self-BLEU by comparing each output against the others
        # For simplicity, we'll compute BLEU-1 for each and average them
        scores = []
        for i in range(len(decoded_texts)):
            ...
            for j in range(len(decoded_texts)):
                if i == j:
                    continue
                candidate = decoded_texts[i]
                references = decoded_texts[j]
                result = self.bleu.compute(predictions=[candidate], references=[references])
                # scores.append(result["bleu"])
                temp_bleu_list.append(result["bleu"])
            scores.append(sum(temp_bleu_list) / len(temp_bleu_list))
        # Average self-BLEU
        self_bleu = sum(scores) / len(scores)
        ...
        loss = ((per_token_loss * completion_mask).sum() / completion_mask.sum()) if self.constant_normalizer is None else ((per_token_loss * completion_mask).sum() / self.constant_normalizer)
        # loss = loss + self_bleu
        ...
        if self.beta != 0.0:
            mean_kl = ((per_token_kl * completion_mask).sum() / completion_mask.sum()) if self.constant_normalizer is None else ((per_token_kl * completion_mask).sum() / self.constant_normalizer)
            self._metrics[mode]["kl"].append(self.accelerator.gather_for_metrics(mean_kl).mean().item())

        is_clipped = (per_token_loss1 < per_token_loss2).float()
        clip_ratio = ((is_clipped * completion_mask).sum() / completion_mask.sum()) if self.constant_normalizer is None else ((is_clipped * completion_mask).sum() / self.constant_normalizer)
        self._metrics[mode]["clip_ratio"].append(self.accelerator.gather_for_metrics(clip_ratio).mean().item())
        # self_bleu_tensor = torch.tensor(self_bleu, device=self.accelerator.device)
        # gathered = self.accelerator.gather_for_metrics(self_bleu_tensor)
        # average_self_bleu = gathered.mean().item()
        # self._metrics[mode]["self_bleu"].append(self.accelerator.gather_for_metrics(self_bleu).mean().item())
        return loss
