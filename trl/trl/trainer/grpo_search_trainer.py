...
from .search.tasks import DynamicSearchTask
from .search.dynamic_search import DynamicBeamSearch
...
class GRPOSearchTrainer(Trainer):
    """
    ...
    def __init__(
        ...
        search_rm_path: Optional[str] = None,
    ):
        # search rm
        self.search_rm_path = search_rm_path        
        ...
        if self.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                print(f"Repetition penalty is {args.repetition_penalty}")
    ...
    def _generate_and_score_completions(
        ...
        prompts = [x["prompt"][-1]['content'] for x in inputs]
        # NOTE: here we need to pass the raw question to our search module. The chat tempalte will be applied in the search module
        ...
        if self.args.use_vllm:
            ...
            # prompt individually.
                ...
                with profiling_context(self, "vLLM.generate"):
                    ...
                    for prompt in ordered_set_of_prompts:
                        ...
                        task.set_models(self.llm, self.processing_class, "", "")
                        task.set_question(prompt)
                        search = DynamicBeamSearch(
                            task,
                            depth=2,
                            target_num_gens=self.args.num_generations,
                            max_length=2048,
                            tau=1.3,
                        )
                        print("Successfully generated search")
                        nodes = search[1].traverse(node_list=[], id=0, pre_id=-1)
                        nodes = sorted(nodes, key=lambda x: x[3], reverse=True)
                        last_depth = nodes[0][3]
                        nodes = [node for node in nodes if node[-1] == True]
                        nodes = nodes[:self.args.num_generations]
                        assert len(nodes) == self.args.num_generations, f"There are only {len(nodes)} nodes available"
                        temp_reward_list = []
                        for node in nodes:
                            completion_ids.append(node[2])
                            # temp_reward_list.append(node[1])
                            all_rewards.append(torch.tensor(node[1], device=device))
                            # print(f"Rewards: {all_rewards}")
                        # all_rewards.append(temp_reward_list)
            ...
            #         completion_ids.append(output.token_ids)
                ...
                all_rewards = [None] * len(all_prompts_text)
            all_rewards = broadcast_object_list(all_rewards, from_process=0)
            ...
            all_rewards = torch.tensor(all_rewards, device=device)
            # all_rewards = all_rewards[process_slice]
        else:
        ...
        # print(f"Final completions: {completions}")
        ...
        # rewards_per_func = torch.cat(all_rewards, dim=0).view(self.reward_weights.shape)
        print(f"Reward weights is: {self.reward_weights.shape}")
        rewards_per_func = all_rewards
