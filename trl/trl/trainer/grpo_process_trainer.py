...
class GRPOProcessTrainer(Trainer):
    """
    ...
    def _generate_and_score_completions(
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            with profiling_context(self, reward_func_name):
                if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                    def remove_boxed(text, num_from_last):
                        segments = text.split("\n\n")
                    ...
                    print(f"Getting rewards from iteration {i}")
                    if is_conversational(inputs[0]):
                        processed_completions = [[{"role": "assistant", "content": remove_boxed(c[0]['content'], i)}] for c in completions]
                        ...
                        processed_completions = [remove_boxed(c, i) for c in completions]
                    ...
                    #     reward_logits = unwrapped_reward_func(**reward_inputs).logits[:, 0]  # Shape (sum(num_trajectories),)

                    # # Aggregate rewards: Mean across available trajectories for each text
                    # split_rewards = torch.split(reward_logits, num_trajectories)  # Split rewards based on actual trajectory counts
                    # rewards_per_func[:, i] = torch.tensor([r.mean() for r in split_rewards], device=reward_logits.device)
                else:
                    # Repeat all input columns (but "prompt" and "completion") to match the number of generations
                    keys = [key for key in inputs[0] if key not in ["prompt", "completion"]]
                    reward_kwargs = {key: [example[key] for example in inputs] for key in keys}
                    # print("Completions in trainer: {}".format(completions))
        ...
        mean_reward_per_func = rewards_per_func.mean(0)
        print(rewards_per_func.shape)
        for i, reward_func in enumerate(self.reward_funcs):
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                reward_func_name = reward_func.config._name_or_path.split("/")[-1]
                self._metrics[mode][f"rewards/{reward_func_name}"].append(mean_reward_per_func[i].item())
            else:
                reward_func_name = reward_func.__name__
                if mode == "train":
                    self._metrics[mode][f"rewards/{reward_func_name}"].append(mean_reward_per_func[i].item())
                    ...
                    # }
                    # for j in range(7):
                    #     self._metrics[mode][f"rewards/{reward_func_name}/{id_2_task_dict[j]}"].append(
                    #         rewards_per_func[i][j:j+3].mean().item()
                    #     )
