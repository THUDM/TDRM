...
def save_question_reward_step(question: str, mean: Union[float, list[float]], std: Union[float, list[float]], global_step: int, folder: str):
    """
    A untility to save each question along with thier reward (mean and std)
    """
    output_path = os.path.join(folder, "question_reward.json")
    with open(output_path, "a") as f:
        if isinstance(mean, list):
            assert isinstance(std, list)
            f.write(json.dumps({
                "question": question,
                "mean": [float(mean_item) for mean_item in mean],
                "std": [float(std_item) for std_item in std],
                "step": global_step
            }))
            f.write("\n")
        else:
