...
def get_inference_model_vllm(model_dir):
    llm = LLM(model=model_dir, gpu_memory_utilization=0.5)
    inference_tokenizer = AutoTokenizer.from_pretrained(model_dir, trust_remote_code=True)

    return inference_tokenizer, llm
...
def get_vllm_response(query: str, model, tokenizer, sampling_args: SamplingParams):
    print(f"Query is {tokenizer.decode(query)}")
    outputs = model.generate(prompt_token_ids=query, sampling_params=sampling_args, use_tqdm=False)
