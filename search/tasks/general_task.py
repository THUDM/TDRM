...
class SearchTask(object):
    def __init__(self, data: str, tokenizer,
        ...
        if inference_model_path is not None:
            self.inference_tokenizer, self.inference_model = get_inference_model_vllm(inference_model_path)
        if value_model_path is not None:
            self.value_tokenizer, self.value_model = get_value_model(value_model_path)
    ...
    def get_next_step(self, y, node_depth, num_branch, stop=None, stop_token_ids=None, max_length=1024, **kwargs) -> list[tuple[str, bool, int]]:
        ...
        if self.system_prompt is not None:
            prompt = self.tokenizer.apply_chat_template([{"role": "system", "content": self.system_prompt}, {"role": "user", "content": self.question}, {"role": "assistant", "content": self.tokenizer.decode(y)}], add_generation_prompt=False)[:-1]
            ...
            prompt = self.tokenizer.apply_chat_template([{"role": "user", "content": self.question}, {"role": "assistant", "content": self.tokenizer.decode(y)}], add_generation_prompt=False)[:-1]
