...
def build_dataset(tokenizer, train_path, eval_path):
    ...
    def tokenize(sample):
        ...
        state3 = sample['conversations']['state3']
        # message = [
        #     {"role":"user", "content":question},
        #     {"role":"assistant", "content":ans}
        # ]
        ...
        message3 = wrap_state(question, state3)
        sample['state3'] = tokenizer.apply_chat_template(
            message3, tokenize=False, add_generation_prompt=False)
        tokenized_pos3 = tokenizer(sample['state3'], truncation=True)
        sample["input_ids_3"] = tokenized_pos3["input_ids"]
        sample["attention_mask_3"] = tokenized_pos3["attention_mask"]

        sample['label'] = sample['conversations']['label']
        sample['reward_1'] = sample['conversations']['reward_1']
        sample['reward_2'] = sample['conversations']['reward_2']
        sample['reward_3'] = sample['conversations']['reward_3']
        sample['done'] = sample['conversations']['done']
        sample['state3_mask'] = sample['conversations']['state3_mask']
        return sample

    ds = load_from_disk(train_path)['train'].shuffle(seed=42)
...
class RewardDataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            ...
            merged_features.append(
                {
                    "input_ids": feature["input_ids_3"],
                    "attention_mask": feature["attention_mask_3"],
                }
            )

            label_list.append([feature['label']])
            done_list.append([feature['done']])
            reward1_list.append([feature['reward_1']])
            reward2_list.append([feature['reward_2']])
            reward3_list.append([feature['reward_3']])
            state3_mask = feature['state3_mask']
        ...
        batch = {
            ...
            "return_loss": True,
            "done": done_list,
            "reward_1": reward1_list,
            "reward_2": reward2_list,
            "reward_3": reward3_list,
            "state3_mask": state3_mask,
        }
        return batch
...
class RewardTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        r1 = inputs['reward_1']
        print(r1)
        r1 = torch.tensor(r1,dtype=torch.bfloat16).to(state1_outputs.logits.device)
        r2 = inputs['reward_2']
        r2 = torch.tensor(r2,dtype=torch.bfloat16).to(state1_outputs.logits.device)
        r3 = inputs['reward_3']
        r3 = torch.tensor(r3,dtype=torch.bfloat16).to(state1_outputs.logits.device)
        state3_mask = inputs['state3_mask']
        state3_mask = torch.tensor(state3_mask,dtype=torch.bfloat16).to(state1_outputs.logits.device)

        state1_q = state1_outputs.logits
        state3_q = state3_outputs.logits.detach()

        state1_probs = torch.sigmoid(state1_q)
        state3_probs = torch.sigmoid(state3_q)
        td_probs = r1 + 0.8 * r2 + 0.8 ** 2 * r3 + 0.8 ** 3 * state3_probs * state3_mask
        td_probs = torch.clamp(td_probs, min=0, max=1)
        # td_state2_probs = 0.8 * torch.sigmoid(state2_q)
        print(f"TD Prob: {td_probs}")
        ...
        loss = done * script_args.terminal_weight * ((label * torch.log(state1_probs+1e-10) + (1 - label) * torch.log(1 - state1_probs+1e-10))) + \
            (1 - done) * (td_probs * torch.log(state1_probs+1e-10) + (1 - td_probs) * torch.log(1 - state1_probs+1e-10))
