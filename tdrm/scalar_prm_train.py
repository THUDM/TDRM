...
class ScriptArguments:
    ...
    train_set_path: Optional[str] = field(
        # default="/workspace/hf_home/hub/datasets/RLHFlow___mistral-prm-data", # only step 1
        default="/path/to/TDRM/outputs/datasets/rlhflow_scalar_prm",
        metadata={"help": "The dir of the subset of the training data to use"},
    )
    output_path: Optional[str] = field(
        default="./models/llama3_scalar_prm_baseline",
...
def build_dataset(tokenizer, train_path, eval_path):
    ...
    def tokenize(sample):
        ...
        if len(sample['conversations'][0]['content'].split("Step 1")) == 1:
            ans = ""
    ...
    ds = load_from_disk(train_path).shuffle(seed=42)['train']
    #ds = ds.select(range(2000))
    ds = ds.map(tokenize, num_proc=48)
...
if accelerator.is_main_process:
    ...
    wandb.init(project="PRM-Training", name=f"{script_args.run_name}_rlhflow_PRM_{script_args.num_train_epochs}epoch_{script_args.learning_rate}lr", entity="miniac-workspace")
